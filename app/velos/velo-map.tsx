...
import type { BikeStation } from "@/lib/velo/nantes";
...
function occupancyColor(station: BikeStation): string {
...
function statusLabel(station: BikeStation): string {
  if (!station.isRenting) return "Station fermée";
  if (station.bikesAvailable <= 0) return "Aucun vélo disponible";
  return `${station.bikesAvailable} vélo${station.bikesAvailable > 1 ? "s" : ""} disponible${station.bikesAvailable > 1 ? "s" : ""}`;
}

type VeloMapProps = {
  initialStations: BikeStation[];
};

export default function VeloMap({ initialStations }: VeloMapProps) {
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      try {
        const response = await fetch("/api/velo", { cache: "no-store" });
        if (!response.ok) return;
        const data = await response.json();
        setStations(data.stations);
      } catch (error) {
        console.error("[VeloMap] refresh failed", error);
      }
    }, REFRESH_INTERVAL_MS);
  ...
  function handleSelect(station: BikeStation) {
